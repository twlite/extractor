...
class AppleMusic {
    ...
    static validateURL(url: string): boolean {
        ...
        if (AppleMusic.linkType(url) == false) return false;
        else return true;
    }

    static linkType(url: string) {
    ...
    static getRawAlbum(document: string): RawAlbum {
    ...
    static getRawPlaylist(document: string): RawPlaylist {
        const $ = cheerio.load(document);
    ...
    static async search(url: string): Promise<RawPlaylist | RawAlbum | Track | null> {
        const urlType = AppleMusic.linkType(url);
        ...
        if (urlType === 'playlist') {
            return AppleMusic.getRawPlaylist(page);
        }

        const album = AppleMusic.getRawAlbum(page);
    ...
    static async getInfo(query: string): Promise<any> {
        const music_data = await AppleMusic.search(query);
        ...
        if (music_data.type === 'song') {
            ...
            return {
                playlist: null,
                data:
                    (info.data as Omit<any, 'playlist'>['data'])?.map((m: any) => ({
